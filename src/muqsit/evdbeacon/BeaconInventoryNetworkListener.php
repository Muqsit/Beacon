...
final class BeaconInventoryNetworkListener {
    public function __construct(Loader $plugin) {
        SimplePacketHandler::createInterceptor($plugin)
            ->interceptOutgoing(static function (
                ...
                if (
                    $origin
                        ->getInvManager()
                        ->getWindow($packet->windowId) instanceof
                    BeaconInventory
                ) {
            ...
            ->interceptIncoming(static function (
                ...
                if ($packet->trData instanceof NormalTransactionData) {
                    foreach ($packet->trData->getActions() as $action) {
                        if ($action->inventorySlot === 27) {
                            ...
                            if (
                                $inv_manager->getWindow($window_id) instanceof
                                BeaconInventory
                            ) {
                                $action->sourceType =
                                    NetworkInventoryAction::SOURCE_CONTAINER;
                                $action->windowId = $window_id;
                                $action->inventorySlot =
                                    BeaconInventory::SLOT_FUEL;
                            }
                        }
                    }
                }
            ...
            ->interceptIncoming(static function (
                ...
                if ($player instanceof Player) {
                    ...
                    if ($tile instanceof Beacon) {
                        ...
                        if (
                            !$fuel->isNull() &&
                            BeaconManager::getInstance()->isFuelItem($fuel)
                        ) {
