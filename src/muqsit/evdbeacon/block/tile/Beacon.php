...
class Beacon extends Spawnable implements InventoryHolder, Nameable {
    ...
    public static function readBeaconEffects(CompoundTag $nbt): array {
        ...
        return [
            self::EFFECT_PRIMARY => $map->fromId(
                $nbt->getInt(self::TAG_PRIMARY, 0)
            ),
            self::EFFECT_SECONDARY => $map->fromId(
                $nbt->getInt(self::TAG_SECONDARY, 0)
            )
        ];
    }
    ...
    public static function getLayerRequirementForBeaconEffect(int $type): int {
        ...
        throw new InvalidArgumentException(
            "Invalid beacon effect type {$type}"
        );
    }
    ...
    public function readSaveData(CompoundTag $nbt): void {
        ...
        $this->setEffects(
            $effects[self::EFFECT_PRIMARY],
            $effects[self::EFFECT_SECONDARY]
        );
    ...
    protected function addBeaconEffectsData(CompoundTag $nbt): void {
        ...
        $nbt->setInt(
            self::TAG_PRIMARY,
            isset($this->effects[self::EFFECT_PRIMARY])
                ? $map->toId($this->effects[self::EFFECT_PRIMARY]->getType())
                : 0
        );
        $nbt->setInt(
            self::TAG_SECONDARY,
            isset($this->effects[self::EFFECT_SECONDARY])
                ? $map->toId($this->effects[self::EFFECT_SECONDARY]->getType())
                : 0
        );
    }
    ...
    public function getValidEffects(): array {
        ...
        foreach ($this->effects as $beacon_effect_type => $effect) {
            if (
                $this->layers >=
                self::getLayerRequirementForBeaconEffect($beacon_effect_type)
            ) {
                $type = $effect->getType();
                if ($beacon_manager->isEffectValid($type, $this->layers)) {
                    ...
                    if (
                        !isset(
                            $effects[($runtime_id = $type->getRuntimeId())]
                        ) ||
                        $amplifier > $effects[$runtime_id]->getAmplifier()
                    ) {
                        $effects[$runtime_id] = new EffectInstance(
                            $type,
                            $effect->getDuration(),
                            $amplifier,
                            $effect->isVisible(),
    ...
    public function setEffects(?Effect $primary, ?Effect $secondary): void {
        ...
        if ($primary !== null) {
            $effects[self::EFFECT_PRIMARY] = new EffectInstance(
                $primary,
        ...
        if ($secondary !== null) {
            $effects[self::EFFECT_SECONDARY] = new EffectInstance(
                $secondary,
                $this->getEffectDuration(),
                1,
                true,
            ...
            for ($y = $this->pos->y + 1; $y <= World::Y_MAX; ++$y) {
                if (
                    ...
                    $iterator->moveTo($x, $y, $z) ===
                        SubChunkExplorerStatus::INVALID
                ) {
                    continue;
                }
                ...
                if (
                    !$block_factory
                        ->fromFullBlock(
                            $iterator->currentChunk->getFullBlock(
                                $x & 0x0f,
                                $y,
                                $z & 0x0f
                            )
            ...
            for ($x = $min_x; $x <= $max_x; ++$x) {
                for ($z = $min_z; $z <= $max_z; ++$z) {
                    if (
                        ...
                        $iterator->moveTo($x, $y, $z) !==
                            SubChunkExplorerStatus::INVALID &&
                        $beacon_manager->isFullBlockPyramidBlock(
                            $iterator->currentChunk->getFullBlock(
                                $x & 0x0f,
                                $y,
                                $z & 0x0f
